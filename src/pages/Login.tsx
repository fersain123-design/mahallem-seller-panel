...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(getAuthErrorMessage(err, 'Giriş başarısız oldu'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthSplitLayout left={<AuthQuotesPanel />}>
      <div className="min-h-[62vh] flex items-center">
          ...
          <div className="mb-6">
            <h1 className="text-xl font-semibold tracking-tight text-text-primary">Giriş yap</h1>
            <p className="text-sm text-text-secondary mt-1">Hesabınıza erişmek için bilgilerinizi girin.</p>
          </div>

          {error && (
            <div className="mb-5 rounded-xl border border-error/25 bg-error/5 p-4 text-error flex items-start gap-2">
